...
class _CoapMessageLayer(object):
    ...
    def _deduplicate_message(self, message):
        ...
        def _add_message_to_recent(cache, key):
            ...
            if self._expiration_timer is None:
                self._expiration_timer = Timer(EXCHANGE_LIFETIME, _remove_outdated_messages, (cache,))
                self._expiration_timer.daemon = True
                self._expiration_timer.start()

        def _remove_outdated_messages(cache):
            ...
            for key, data in cache.items():
                ...
                if timeout <= now:
                    del cache[key]
                elif (timeout - now).total_seconds() < next_timeout:
                    next_timeout = (timeout - now).total_seconds()

            if not bool(cache):
                self._expiration_timer = None
            else:
                self._expiration_timer = Timer(next_timeout, _remove_outdated_messages, (cache,))
                self._expiration_timer.daemon = True
                self._expiration_timer.start()
